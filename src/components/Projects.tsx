...
const Projects = ({ onExpandedChange, isExpanded: externalExpanded, onExpandRequest }: ProjectsProps) => {
  ...
  const getColorHex = (color: string) => {
    switch (color) {
      case 'yellow': return '#D4D994';
      case 'magenta': return '#CB7885';
      case 'cream': return '#F5F5F0';
      case 'orange': return '#5E6623';
  ...
  const getTextColor = (color: string) => {
    switch (color) {
      case 'yellow': return '#893941';
      case 'magenta': return 'white';
      case 'cream': return '#5E6623';
      case 'orange': return 'white';
      default: return '#5E6623';
    }
  };
  ...
  return (
    <section id="projects" className="relative py-20 px-6" style={{ backgroundColor: '#893941' }}>
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <motion.h2 
            ...
            style={{ 
              color: '#D4D994',
              textShadow: isSectionExpanded ? '0 0 20px rgba(212, 217, 148, 0.5), 0 0 40px rgba(212, 217, 148, 0.3)' : '0 0 0px rgba(212, 217, 148, 0)'
            }}
            onMouseEnter={(e) => {
              if (!isSectionExpanded) {
                e.currentTarget.style.textShadow = '0 0 20px rgba(212, 217, 148, 0.5), 0 0 40px rgba(212, 217, 148, 0.3)';
              }
            }}
            onMouseLeave={(e) => {
              if (!isSectionExpanded) {
          ...
          >
            ...
            <svg className="absolute -bottom-3 left-0 w-full" height="15" viewBox="0 0 400 15">
              <path d="M0,10 Q100,0 200,10 T400,10" stroke="#D4D994" strokeWidth="6" fill="none" strokeLinecap="round"/>
            </svg>
          </motion.h2>
        </div>
              ...
              >
                ...
                <AnimatePresence mode="wait">
                  {!isExpanded ? (
                    ...
                    >
                      <h3 className="text-2xl font-black mb-2" style={{ color: '#893941' }}>
                        {project.title}
                      </h3>
                ...
                <motion.div
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        <h3 className="text-3xl font-black mb-2" style={{ color: '#893941' }}>
                          {project.title}
                        </h3>
                    ...
                    {project.id === 'asteria1' && project.stats && (
                      <div className="bg-gray-100 rounded-2xl p-4 space-y-2">
                        <h4 className="text-xl font-black" style={{ color: '#893941' }}>Some Stats...</h4>
                        <div className="space-y-1 text-sm text-gray-700">
                          <p><span className="font-bold" style={{ color: '#893941' }}>Max Altitude:</span> {project.stats.maxAltitude.ft}ft [or] {project.stats.maxAltitude.m}m</p>
                          <p><span className="font-bold" style={{ color: '#893941' }}>Max Velocity:</span> {project.stats.maxVelocity.mph} mph [or] {project.stats.maxVelocity.mps} m/s</p>
                          <p><span className="font-bold" style={{ color: '#893941' }}>Max G forces:</span> {project.stats.maxG} Gs</p>
                          <p><span className="font-bold" style={{ color: '#893941' }}>Motor Used:</span> {project.stats.motor}</p>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="flex gap-3 flex-wrap">
                      {project.links?.github && (
                        <a
                          ...
                          style={{ 
                            backgroundColor: project.bgColor === 'magenta' ? 'white' : '#893941',
                            color: project.bgColor === 'magenta' ? '#CB7885' : 'white'
                          }}
                        >
