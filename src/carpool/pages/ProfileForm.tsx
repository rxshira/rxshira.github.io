...
import { MapPin, Phone, Car, Clock, Info, Upload, FileText, CheckCircle, Briefcase, AlertCircle } from 'lucide-react';
...
const ProfileForm = () => {
  ...
  // Track flagged fields from admin rejection
  const flaggedFields = carpoolUser?.rejection_reasons || [];
  const isFixing = carpoolUser?.access_status === 'rejected' && carpoolUser?.rejection_action === 'fix';
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      alert("Security: Only images (JPG, PNG, WEBP) accepted.");
      return;
    }
    setUploading(true);
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.zip_code.trim().length < 5) return alert("Valid Zip Code is required.");
    if (!formData.offer_letter_url) return alert("Offer letter image is required for verification.");
    ...
    try {
      ...
      if (apiKey && (formData.address || formData.zip_code)) {
        try {
          const res = await GoogleDistanceService.geocode(formData.address || formData.zip_code, apiKey);
          if (res) coords = res;
        } catch (e) {}
      }
      ...
      await setDoc(userRef, {
        ...
        access_status: 'pending', // Re-enter pending state
  ...
  // Helper for glowing class
  const getGlow = (key: string) => flaggedFields.includes(key) ? 'ring-2 ring-pink shadow-[0_0_15px_rgba(255,0,110,0.4)] border-pink' : 'border-white/10';

  return (
    <div className="min-h-screen pt-24 pb-12 bg-black px-6 overflow-y-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto">
        <div className="mb-10 text-center">
          <h1 className="text-4xl font-bold mb-2 uppercase tracking-tighter text-white">IBM <span className="text-pink neon-text">Profile</span></h1>
          <p className="text-white/40 text-[10px] uppercase tracking-[0.3em] font-mono font-bold">Secure Verification System</p>
        </div>

        {isFixing && (
          <div className="mb-8 bg-pink/10 border border-pink/30 p-6 rounded-sm flex items-start gap-4">
            <AlertCircle className="w-5 h-5 text-pink shrink-0 mt-0.5" />
            <div>
              <h4 className="text-pink font-bold uppercase text-[11px] tracking-widest mb-1">Action Required</h4>
              <p className="text-white/70 text-[10px] font-mono leading-relaxed italic">
                Your previous submission was flagged. Please review and correct the highlighted sections below.
                {carpoolUser?.rejection_reason && <span className="block mt-2 text-white not-italic">Note: "{carpoolUser.rejection_reason}"</span>}
              </p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-8 bg-white/5 border border-white/10 p-8 md:p-12 rounded-sm shadow-2xl">
          {/* Offer Letter */}
          <div className={`space-y-4 p-1 rounded-sm transition-all ${getGlow('offer')}`}>
            <h3 className="text-white font-bold uppercase text-xs tracking-widest flex items-center gap-2 font-mono"><FileText className="w-4 h-4 text-pink" /> Verification</h3>
          ...
          <div className={`space-y-4 pt-4 border-t border-white/5 transition-all ${getGlow('name')}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[10px] text-pink font-bold uppercase font-mono px-1">Full Name (Mandatory)</label>
                <input required className="w-full bg-black/50 border border-white/10 p-3 focus:border-pink outline-none text-sm font-mono text-white" value={formData.full_name} onChange={e => setFormData({...formData, full_name: e.target.value})} />
              </div>
              <div className={`space-y-1 transition-all ${getGlow('phone')}`}>
          ...
          {/* Location */}
          <div className={`space-y-4 pt-4 border-t border-white/5 transition-all ${getGlow('address')}`}>
            ...
            <div className="flex flex-col gap-4 p-4 bg-white/5 border border-white/5">
              ...
              {formData.has_car && (
                <div className="flex items-center gap-6 border-t border-white/5 pt-4">
                  <div className="flex items-center gap-3 text-white"><span className="text-[10px] text-white/40 uppercase font-mono">Seats:</span><select className="bg-black/50 border border-white/10 p-1 text-xs outline-none font-mono" value={formData.seats_available} onChange={e => setFormData({...formData, seats_available: parseInt(e.target.value)})}>{[1,2,3,4,5,6].map(n => <option key={n} value={n}>{n}</option>)}</select></div>
                  <div className="flex-1 space-y-1">
                    <label className="text-[9px] text-white/40 uppercase font-mono">Vehicle Details</label>
